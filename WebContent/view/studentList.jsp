	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns:
				...
				{field:'chk',	checkbox: true,	width:50},
 		        {field:'id',	title:'ID',		width:50, 	sortable: true}, //可排序   
				...
				{field : 'clazz_id',title : '班级',width : 150,
					formatter : function(value, row,index) { //查询班级
					if (row.clazzId) {var clazzList = $("#clazzList").combobox("getData");
			...
			$("#delete").click(function() {
				...
				if (selectLength == 0) {
					...
					$.messager.confirm("消息提醒","将删除与学生相关的所有数据(包括成绩)，确认继续？",
					function(r) {
						if (r) {
							$.ajax({
								type : "post",
			...
			$("#addDialog").dialog(
			{
				...
				width : 	650,
				height : 	460,
				iconCls : 	"icon-add",//窗口小图标
				modal : 	true,//置顶
				collapsible : false,//不可折叠
				minimizable : false,//无最小化
				maximizable : false,//无最大化
				draggable : true,//可移动
				closed :	true,//有关闭
				buttons : [
				{	
							...
							$.ajax({
								...
								success : function(msg) {
									if (msg == "success") {
										...
										$('#dataList').datagrid("reload");
										setTimeout( function() { $("#clazzList").combobox('setValue',clazzid);}, 100 );
									} else {
			...
			onBeforeOpen : function() {
				...
				setTimeout(function() {$("#edit_clazzList").combobox('setValue', clazzid);}, 100);
			}
		});
