...
import com.iyeee.programmer.dao.CourseDao;
import com.iyeee.programmer.dao.ScoreDao;
import com.iyeee.programmer.dao.SelectedCourseDao;
import com.iyeee.programmer.dao.StudentDao;
import com.iyeee.programmer.model.Course;
import com.iyeee.programmer.model.Page;
import com.iyeee.programmer.model.Score;
...
public class ScoreServlet extends HttpServlet {
	...
	private void getStatsList(HttpServletRequest request,
		...
		List<Map<String, Object>> scoreList = scoreDao.getGradeList(score);
	...
	private void importScore(HttpServletRequest request,
		...
		try {
			...
			for(int rowNum = 1; rowNum <= sheetAt.getLastRowNum(); rowNum++){
				...
				String scoreValue = cell.getStringCellValue();
				//获取第3列，备注
				cell = row.getCell(3);
				String remark = null;
				if(cell != null){
	...
	private void getGradeList(HttpServletRequest request,
							  HttpServletResponse response) {
		// TODO Auto-generated method stub
		int studentId = request.getParameter("studentid") == null ? 0 : Integer.parseInt(request.getParameter("studentid").toString());
		int courseId = request.getParameter("courseid") == null ? 0 : Integer.parseInt(request.getParameter("courseid").toString());
		...
		score.setStudentId(studentId);
		ScoreDao scoreDao = new ScoreDao();
		List<Score> courseList = scoreDao.getGradeList(score, new Page(currentPage, pageSize));
		int total = scoreDao.getGradeListTotal(score);
		scoreDao.closeCon();
		response.setCharacterEncoding("UTF-8");
	...
	private void addScore(HttpServletRequest request,
		...
		int studentId = request.getParameter("studentid") == null ? 0 : Integer.parseInt(request.getParameter("studentid").toString());
		int courseId = request.getParameter("courseid") == null ? 0 : Integer.parseInt(request.getParameter("courseid").toString());
		String scoreNum =request.getParameter("grade");
		String remark = request.getParameter("remark");
		Score score = new Score();
		score.setCourseId(courseId);
		score.setStudentId(studentId);
		score.setGrade(scoreNum);
		score.setRemark(remark);
		ScoreDao scoreDao = new ScoreDao();
		if(scoreDao.isAdd(studentId, courseId)){
			try {
