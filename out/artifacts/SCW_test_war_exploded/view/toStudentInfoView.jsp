        ...
        $(function() {
            ...
            $('#dataList').datagrid({
                ...
                onLoadSuccess:function () {
                    ...
                    // $('#dataList').datagrid('reloadFooter',[{chk: "",type:'合计总费用',id:"￥"+compute(),courseId: ""}]);
                }
            });
            //提前加载学生和课程信息
            ...
            function compute() {
                ...
                for (var i = 0; i < rows.length; i++) {
                    // console.log(rows[i])
                    // console.log(rows[i].courseId)
                    if(rows[i].kind==1) {
                        total += parseInt(getcost(rows[i].courseId));
                    }
                }
                // console.log(total);
                return total;
            ...
            function getcost(courseId) {
                var courseList = $("#courseList").combobox("getData");
                for(var i=0;i<courseList.length;i++ ) {
                    console.log(courseList[i]);
                    if (courseId == courseList[i].id)
                        return courseList[i].cost;
                }
            }
